 ...
 *  - Receiving E2/EA templates (decision required)
 *  - Sending Continue Transaction [DE, D2] with selectable decision
 *
 * Usage:
 *   java -jar vipa-client.jar <vipaHost> <vipaPort> [DECISION]
 *
...
public class VIPAClient {
    ...
    public void run() {
        try (Socket socket = new Socket(host, port)) {
            System.out.println("Connected to VIPA terminal " + host + ":" + port);
            InputStream in = socket.getInputStream();
            OutputStream out = socket.getOutputStream();
            
            // After connecting, read unsolicited wake-up frame first
            byte[] first = readVipaFrame(in);
            if (first != null) {
                int cla = first[3] & 0xFF;
                int ins = first[4] & 0xFF;
                if (cla == 0xE6 && ins == 0x21) {
                	System.out.println(">> Received: " + bytesToHex(first));
...
//            out.flush();
    ...
    private static byte[] buildStartTransactionExample() {
        ...
        e0.write(0x9A); e0.write(0x03); e0.write(0x01); e0.write(0x12); e0.write(0x25);
        // 9F21 (time) 03 HH MM SS
        e0.write(0x9F); e0.write(0x21); e0.write(0x03); e0.write(0x12); e0.write(0x30); e0.write(0x00);
        // 9C Transaction type
        e0.write(0x9C); e0.write(0x01); e0.write(0x00);
        // 9F02 amount (6 bytes) - example 0000000009895 -> we'll use 00000000009895 and take last 6 bytes
        byte[] amt = hexStringToBytes("00000000009895");
        byte[] amt6 = Arrays.copyOfRange(amt, amt.length - 6, amt.length);
        e0.write(0x9F); e0.write(0x02); e0.write(0x06);
        try { e0.write(amt6); } catch (IOException ignored) {}
        ...
        try { data.write(e0b); data.write(0x01); data.write(0x00); } catch (IOException ignored) {}

        byte[] dataBytes = data.toByteArray();
        body.write((byte) dataBytes.length);
        try { body.write(dataBytes); } catch (IOException ignored) {}

        byte[] bodyBytes = body.toByteArray();
        // len = number of bytes AFTER LEN and BEFORE LRC
        int len = bodyBytes.length;
        bout.write((byte) len);
        try { bout.write(bodyBytes); } catch (IOException ignored) {}
    ...
    public static byte[] buildContinueTransaction(ContinueDecision decision, byte[] incomingPayload) {
        ...
        try {
            switch (decision) {
                case DECLINE:
                    ...
                    tlvs.write(0x8A); tlvs.write(0x02); tlvs.write(0x05); tlvs.write(0x00);
                    break;
                case APPROVE_OFFLINE:
                    tlvs.write(0x8A); tlvs.write(0x02); tlvs.write(0x30); tlvs.write(0x30);
                    byte[] iad = hexStringToBytes("11223344556677889900");
                    tlvs.write(0x91); tlvs.write(iad.length); tlvs.write(iad);
                    break;
                case APPROVE_ONLINE:
                    tlvs.write(0x8A); tlvs.write(0x02); tlvs.write(0x30); tlvs.write(0x30);
                    byte[] iad2 = hexStringToBytes("8877665544332211AABB");
                    tlvs.write(0x91); tlvs.write(iad2.length); tlvs.write(iad2);
                    byte[] script = hexStringToBytes("860F842400000000010000000000000000");
                    tlvs.write(0x71); tlvs.write(script.length); tlvs.write(script);
                    break;
                case MINIMAL:
                    // no TLVs
                    break;
        ...
        try { body.write(tlvBytes); } catch (IOException ignored) {}

        byte[] bodyBytes = body.toByteArray();
        int len = bodyBytes.length + 1;
        bout.write((byte) len);
        try { bout.write(bodyBytes); } catch (IOException ignored) {}
